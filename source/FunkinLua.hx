		...
		Lua_helper.add_callback(lua, "addChromaticAbberationEffect", function(camera:String, chromeOffset:Float = 0.005)
			{
				...
				{
					PlayState.instance.addShaderToCamera(camera, new Shaders.GlitchEffectP(waveSpeed, waveFrq, waveAmp));
				});
				...
				{
					PlayState.instance.addShaderToCamera(camera, new Shaders.PulseEffectP(waveSpeed, waveFrq, waveAmp));
				});
				...
				{
					PlayState.instance.addShaderToCamera(camera, new Shaders.DistortBGEffectP(waveSpeed, waveFrq, waveAmp));
				});
				Lua_helper.add_callback(lua, "addInvertEffect", function(camera:String, lockAlpha:Bool = false)
				{
					PlayState.instance.addShaderToCamera(camera, new Shaders.InvertColorsEffectP(lockAlpha));
				});
				Lua_helper.add_callback(lua, "addGreyscaleEffect", function(camera:String)
				{ // for dem funkies

					PlayState.instance.addShaderToCamera(camera, new Shaders.GreyscaleEffect());
				});
				...
				{ // for dem funkies
					...
					PlayState.instance.addShaderToCamera(camera, new Shaders.ThreeDEffect(xrotation, yrotation, zrotation, depth));
				});
